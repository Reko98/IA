...
GLUI_StaticText * info0, * info1, * info2, * info3,* info4, * info5, * info6, * info7, * info8, * info9, *info10, *info11, *info12;
...
void keyboard(unsigned char key, int x, int y)
{
    ...
    case 'w': //UP
      monitor.setPasos(1);
      lanzar_motor_juego(0);
      cout << "Avanzar\n";
      break;

    case 's': // GIVE
      monitor.setPasos(1);
      lanzar_motor_juego(8);
      cout << "Dar\n";
      break;

    case 'd': // DCHA
      monitor.setPasos(1);
      lanzar_motor_juego(2);
      cout << "Derecha\n";
      break;
  ...
  void display_vistIU(){
    ...
    if(monitor.jugar() and monitor.numero_entidades() > 0){
      ...
      if(strs.size() > 9){
        ...
        info11-> set_text(strs[11].c_str());
        info12-> set_text(strs[12].c_str());
...
void lanzar_motor_grafico(int argc, char **argv){
    ...
    //Main Window
    ventanaPrincipal = glutCreateWindow("Belkan II");
    ...
    info11 = panelIU -> add_statictext_to_panel(panelInfo, "");
    info12 = panelIU -> add_statictext_to_panel(panelInfo, "");
